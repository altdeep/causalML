...
In this section we will focus on how we can learn the parameters of the network if our data is continous.
...
Now that we have learned about different algorithms to learn the network structure from data and we know how to deal with discrete and continous data, lets look at some examples:

## Insurance data

Insurance is a network for evaluating car insurance risks. This discrete data is available in bnlearn package. This data contains the following 27 variables:
...
This is the correct structure of the network or in other words the ground truth. In reality we don't have the ground truth and our goal is to estimate the ground truth structure from data.

```{r, fig.cap="Ground truth network", fig.align="center"}
...
Usually the structure learning algorithms perform poorly in detecting the true structure of network from observational data unless the number of variables are small or there is no latent confounders in the model. Let's apply some of the algorithms that we learned to this data set, assuming that we don't know the ground truth and want to infer it from data.

```{r}
# PC algorithm
learned_pc <- pc.stable(insurance)
graphviz.plot(learned_pc)
```

Let's compute the hamming distance between the ground truth network and the learned network. Hamming distance counts the number of misspecify edges by comparing the structure of the learned graph with the structure of the true graph.

```{r}
hamming(learned = learned_pc, true = dag)
```

As you can see, there are a lot of edges that were not learned correctly in our example. Try producing Gaussian data for a simple small network and then apply the pc algorithm again. You will see that the algorithm performs much better.
