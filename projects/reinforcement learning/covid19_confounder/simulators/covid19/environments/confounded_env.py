...
def get_intervention(action, time):
    ...
    non_adherence_propensity = np.random.choice([0.1, 0.01], p=[0.4, 0.6])
...
def get_reward(intervention, state, time):
    ...
    economic_output_per_time = np.random.choice([10, 100000], p=[0.4, 0.6])
...
register(
    id="COVID19-CONFOUNDED-v0", entry_point=Covid19Env, max_episode_steps=150,
